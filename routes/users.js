...
router.post('/register', function(req, res) {
  ...
  data.save(function(err){
   if(err){
    ...
    res.render('register', { message: 'Data saved successfully!'});
   } 
  })
 });
...
router.post('/edit/:id', function(req, res) {
  User.findByIdAndUpdate(req.params.id, req.body, function (err) {
    if(err){
      req.flash('errorMsg', 'Something went wrong! Data could not updated.');
      res.redirect('/users/edit/'+req.params.id);
  } else {
    req.flash('successMsg', 'Data updated successfully.');
    res.redirect('/users/edit/'+req.params.id);
  }
  });
});
...
router.get('/destroy/:id', function(req, res) {
  User.findByIdAndRemove(req.params.id, function (err, project) {
    if (err) {
      req.flash('errorMsg', 'Data not deleted successfully.');
      res.redirect('/users');
    } else {
      req.flash('successMsg', 'Data deleted successfully.');
      res.redirect('/users');
    }
  });
});
